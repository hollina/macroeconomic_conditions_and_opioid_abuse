...
// Drop if Washington D.C. (since it gets omitted with State by Year FE)
...
// Drug Deaths by Race		
	...
	reghdfe drugwr unemp_rate   [aweight=popw], absorb(countycode state_year year) vce(cluster countycode)  	keepsingletons
		    sum drugwr, meanonly
			scalar Mean = r(mean)
			estadd scalar Mean			
	...
	reghdfe drugbr unemp_rate    [aweight=popb] if popb!=0 , absorb(countycode state_year year) vce(cluster countycode)  	keepsingletons
		    sum drugbr, meanonly
			scalar Mean = r(mean)
			estadd scalar Mean		
	...
	reghdfe drughr unemp_rate   [aweight=poph] if poph!=0 , absorb(countycode state_year year) vce(cluster countycode)  	keepsingletons
		    sum drughr, meanonly
			scalar Mean = r(mean)
			estadd scalar Mean
	...
	reghdfe aopioidwr unemp_rate    [aweight=popw], absorb(countycode state_year year) vce(cluster countycode)  	keepsingletons
	...
	reghdfe aopioidbr unemp_rate   [aweight=popb] if popb!=0, absorb(countycode state_year year) vce(cluster countycode)  	keepsingletons
	...
	reghdfe aopioidhr unemp_rate   [aweight=poph] if poph!=0, absorb(countycode state_year year) vce(cluster countycode)  	keepsingletons
	...
	esttab op_all op_white  op_black op_hisp using "$results_path/tables/table_4_county_by_race.tex" , ///
	...
	local lbl2: variable  label unemp_rate
 	...
 	esttab drug_all drug_white  drug_black drug_hisp  using "$results_path/tables/table_4_county_by_race.tex" , ///
	keep(unemp_rate)  label star(* 0.10 ** 0.05 *** 0.01) ///
...
//Drug Overdoses 
			...
			reghdfe r_drug_ovr__r_w unemp_rate    [aweight=pop_white], absorb(fips state_year year) vce(cluster fips)   
			...
			reghdfe r_drug_ovr__r_b unemp_rate    [aweight=pop_black], absorb(fips state_year year) vce(cluster fips)   
			...
			reghdfe r_drug_ovr__r_h hisp_unemp_rate    [aweight=pop_hisp], absorb(fips state_year year) vce(cluster fips)   
			*sum r_drug_ovr__r_h, meanonly
			*scalar Mean = r(mean)
			scalar Mean = .		
			estadd scalar Mean		
...
//Opioid Overdoes
			...
			reghdfe r_op_ovr__r_w unemp_rate    [aweight=pop_white], absorb(fips state_year year) vce(cluster fips)   
			...
			reghdfe r_op_ovr__r_b unemp_rate    [aweight=pop_black], absorb(fips state_year year) vce(cluster fips)   
			...
			reghdfe r_op_ovr__r_h hisp_unemp_rate    [aweight=pop_hisp], absorb(fips state_year year) vce(cluster fips)   
		    *sum r_op_ovr__r_h, meanonly
			*scalar Mean = r(mean)
			scalar Mean = .
			estadd scalar Mean							
				estadd local year_dum "Yes"
	...
	local lbl2: variable  label unemp_rate		
			...
			esttab ovr_all ovr_white  ovr_black ovr_hisp  using "$results_path/tables/table_4_county_by_race.tex" , ///
			keep(unemp_rate)  label star(* 0.10 ** 0.05 *** 0.01) ///
			append se order(unemp_rate) ///
			booktabs b(%20.2f) se(%20.2f) eqlabels(none) alignment(S S)  ///
			stats(Mean N , fmt(%3.2f 0  ) layout("\multicolumn{1}{c}{@}" "\multicolumn{1}{c}{@}") label("\hspace{0.5cm}Mean of Dependent Variable" "\hspace{0.5cm}Observations")) ///
	...
	esttab dep_all dep_white  dep_black dep_hisp using "$results_path/tables/table_4_county_by_race.tex" , ///
	keep(unemp_rate)  label star(* 0.10 ** 0.05 *** 0.01) ///
	append se order(unemp_rate) ///
	booktabs b(%20.2f) se(%20.2f) eqlabels(none) alignment(S S)  ///
